package com.chf.example.apache.zest.rest.resource;
...
import com.chf.example.apache.zest.rest.Todo;
...
public class Todos {
    ...
    public List<Todo> list(@QueryParam("q") String q, @QueryParam("c") Boolean c) {
        try (UnitOfWork uow = module.newUnitOfWork(newUsecase("List TODOs"))) {
            QueryBuilder<Todo> queryBuilder = module
                    .newQueryBuilder(Todo.class);
            Todo queryTemplate = templateFor(Todo.class);
            if (q != null && q.length() > 0) {
                queryBuilder = queryBuilder.where(matches(
                        queryTemplate.title(), String.format(".*%s.*", q)));
            }
            if (c != null) {
                queryBuilder = queryBuilder.where(eq(queryTemplate.completed(),
                        c));
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Todo update(Todo todo) throws UnitOfWorkCompletionException {
