...
public class MyApplicationAssembler implements ApplicationAssembler {
    ...
    private ModuleAssembly assembleConfiguration(LayerAssembly configLayer)
        ...
        ModuleAssembly configServices = configLayer
                .module("Configuration Services Module");
        ...
        new FileConfigurationAssembler().withOverride(fileConfigOverride)
                .visibleIn(Visibility.application).assemble(configServices);
        new JacksonValueSerializationAssembler().visibleIn(Visibility.layer)
                .assemble(configServices);
        new MemoryEntityStoreAssembler().visibleIn(Visibility.layer).assemble(
                configServices);

        return configLayer.module("Configurations Module");
    }

    private void assembleStorage(LayerAssembly storageLayer,
            ModuleAssembly configModule) throws AssemblyException {
        // Serialization
        new JacksonValueSerializationAssembler().visibleIn(Visibility.layer)
                .assemble(storageLayer.module("Serialization Module"));
        // EntityStore
        new FileEntityStoreAssembler().identifiedBy("file-entitystore")
                .visibleIn(Visibility.application)
                .withConfig(configModule, Visibility.application)
                .assemble(storageLayer.module("EntityStore Module"));

        new RdfNativeSesameStoreAssembler(Visibility.application,
                Visibility.application).assemble(storageLayer
                .module("Index/Query Module"));
    ...
    private void assembleConnectivity(LayerAssembly connectivityLayer)
            throws AssemblyException {
        new JacksonValueSerializationAssembler().assemble(connectivityLayer
                .module(MODULE_REST));
    }
